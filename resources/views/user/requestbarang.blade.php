...
async function loadItemsByKategori(selectKategori, targetSelect) {
    ...
    try {
        ...
        items.forEach(item => {
            ...
            option.value = item.nama; // ✅ Sekarang pakai `nama`
        ...
        // Load tipe/deskripsi berdasarkan kategori
        async function loadTipeByKategori(selectKategori, targetSelect) {
            const kategori = selectKategori.value;
            if (!kategori) return;

            try {
                const response = await fetch(`/requestbarang/api/tipe-barang?kategori=${kategori}`);
